...
vidSwitch.prototype.init = async function () {
	...
	this.xform0 = new imageProcess(
		...
		this.width,
		this.height,
		new transform({ width: this.width, height: this.height })
	)
	await this.xform0.init()
	this.rgbaXf0 = await this.context.createBuffer(numBytesRGBA, 'readwrite', 'coarse', {
		width: this.width,
		height: this.height
	})
	...
	if (this.numInputs > 1) {
		...
		this.rgbaXf1 = await this.context.createBuffer(numBytesRGBA, 'readwrite', 'coarse', {
			width: this.width,
			height: this.height
		})
	...
	this.combiner = new imageProcess(
		...
		this.width,
		this.height,
		new combine({ numOverlays: this.numOverlays })
	)
	await this.combiner.init()
	this.rgbaMx = await this.context.createBuffer(numBytesRGBA, 'readwrite', 'coarse', {
		width: this.width,
		height: this.height
	})
}
