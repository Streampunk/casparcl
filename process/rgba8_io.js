...
function dumpBuf(buf, width, numLines) {
  ...
  let lineOff = 0;
  function getRHex(off) { return buf.readUInt8(lineOff + off * 4 + 0).toString(16); }
  function getGHex(off) { return buf.readUInt8(lineOff + off * 4 + 1).toString(16); }
  function getBHex(off) { return buf.readUInt8(lineOff + off * 4 + 2).toString(16); }
  function getAHex(off) { return buf.readUInt8(lineOff + off * 4 + 3).toString(16); }

  for (let l = 0; l < numLines; ++l) {
    lineOff = pitchBytes * l;
    console.log(`Line ${l}: ${getRHex(0)}, ${getGHex(0)}, ${getBHex(0)}, ${getAHex(0)}; ${getRHex(1)}, ${getGHex(1)}, ${getBHex(1)}, ${getAHex(1)}; ${getRHex(2)}, ${getGHex(2)}, ${getBHex(2)}, ${getAHex(2)}; ${getRHex(3)}, ${getGHex(3)}, ${getBHex(3)}, ${getAHex(3)}`);
  }
}
