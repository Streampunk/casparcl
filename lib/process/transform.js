...
`;
class Transform extends imageProcess_1.ProcessImpl {
    constructor(clContext, width, height) {
        ...
        this.transformArray = colourMaths_1.matrixFlatten(this.transformMatrix);
    }
    async updateMatrix(clQueue) {
        if (!this.matrixBuffer)
            throw new Error('Transform needs to be initialised');
        this.transformArray = colourMaths_1.matrixFlatten(this.transformMatrix);
    ...
    async init() {
        this.matrixBuffer = await this.clContext.createBuffer(this.transformArray.byteLength, 'readonly', 'coarse');
        return this.updateMatrix(this.clContext.queue.load);
    }
    async getKernelParams(params, clQueue) {
        ...
        this.transformMatrix = colourMaths_1.matrixMultiply(colourMaths_1.matrixMultiply(colourMaths_1.matrixMultiply(scaleMatrix, translateMatrix), rotateMatrix), projectMatrix);
