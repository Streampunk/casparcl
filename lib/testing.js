...
const beamcoder_1 = __importDefault(require("beamcoder"));
const nodencl_1 = __importDefault(require("nodencl"));
...
async function init() {
    ...
    const deviceIndex = 0;
    const context = new nodencl_1.default.clContext({
    ...
    const rgbaDst = await context.createBuffer(numBytesRGBA, 'readwrite', 'coarse', { width: width, height: height }, 'me');
    const imageDst = await context.createBuffer(numBytesRGBA, 'readwrite', 'coarse', { width: width, height: height }, 'me');
    context.logBuffers();
    ...
    await beamcoder_1.default.makeSources(params);
    // const beamStreams = await beamcoder.makeStreams(params)
    console.dir(params, { getters: true, depth: 4 });
    const demuxer = await beamcoder_1.default.demuxer({
    ...
    const muxer = beamcoder_1.default.muxer({ name: 'mxf', filename: 'test.mxf' });
