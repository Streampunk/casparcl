...
async function run() {
	...
	const v210Loader1 = new rgbyuv.yuvLoader(
		context,
		colSpecRead,
		colSpecWrite,
	...
	const v210Loader2 = new rgbyuv.yuvLoader(
		context,
		colSpecRead,
		colSpecWrite,
		new v210_io.reader(width, height)
	...
	async function processFrame(b1, b2, fl) {
		...
		res[2] = await testImageProgram.run({
			input1: rgbaDst1,
			input2: rgbaDst2,
			fl: fl,
			output: imageDst
	...
	let frameList1 = await fs.readdir(
		'../media/EBU_test_sets/filexchange.ebu.ch/EBU test sets - Creative Commons (BY-NC-ND)/HDTV test sequences/1080i25/Graphics_1080i_/Graphics_1080i_'
	)
	frameList1 = frameList1.filter((f) => f.endsWith('v210'))
	let frameList2 = await fs.readdir(
		'../media/EBU_test_sets/filexchange.ebu.ch/EBU test sets - Creative Commons (BY-NC-ND)/HDTV test sequences/1080i25/girlflower1_1080i_/girlflower1_1080i_'
	)
	...
	while (true) {
		let fh1 = await fs.open(
			'../media/EBU_test_sets/filexchange.ebu.ch/EBU test sets - Creative Commons (BY-NC-ND)/HDTV test sequences/1080i25/Graphics_1080i_/Graphics_1080i_/' +
				frameList1[counter % frameList1.length],
			'r'
		)
		let fh2 = await fs.open(
			'../media/EBU_test_sets/filexchange.ebu.ch/EBU test sets - Creative Commons (BY-NC-ND)/HDTV test sequences/1080i25/girlflower1_1080i_/girlflower1_1080i_/' +
				frameList2[counter % frameList2.length],
			'r'
		)
