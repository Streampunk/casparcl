...
async function run() {
	...
	let pipel = H.pipeline(
		H.flatMap((p) => {
			console.log('DECODE', process.hrtime(stamp))
			return H(dec.decode(p))
		}),
		H.flatMap((p) => {
			console.log('ENCODE', process.hrtime(stamp))
			return H(enc.encode(p.frames))
		}),
		H.flatMap((p) => {
			console.log('PROCESS', process.hrtime(stamp))
			return H(processFrame(p.packets[0].data))
		})
	)
